...
public class QueryExecutor {
	...
	/**
	 * Establishes and returns a database connection. Throws QueryExecutorException
	 * if connection fails.
	 */
	private Connection getConnection() throws QueryExecutorException {
		try {
			...
			throw new QueryExecutorException(ErrorCode.DATABASE_CONNECTION_ERROR,
					"Failed to establish database connection", e);
		}
	}

	/**
	 * Starts a new database transaction. Throws QueryExecutorException if
	...
	public int executeUpdate(QueryBuilder query) throws QueryExecutorException {
		...
		try (PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
			preparedStatementHelper(preparedStatement, query);
			...
			throw new QueryExecutorException(ErrorCode.QUERY_EXECUTION_FAILED, "Error executing update query: " + sql,
					e);
		} finally {
			if (!isTransaction) {
				closeConnection();
	...
	private void preparedStatementHelper(PreparedStatement preparedStatement, QueryBuilder queryBuilder)
			throws SQLException {
		List<Object> queryParams = queryBuilder.query.getQueryParams();
	...
	public Pair executeUpdateWithGeneratedKeys(QueryBuilder sql) throws QueryExecutorException {
		...
		System.out.println(sql.query.getQueryParams());
		try (PreparedStatement preparedStatement = connection.prepareStatement(sql.build(),
				Statement.RETURN_GENERATED_KEYS)) {
			preparedStatementHelper(preparedStatement, sql);
			int rowCount = preparedStatement.executeUpdate();
			int generatedKey = -1;
	 ...
	 * @throws QueryExecutorException if query execution fails
	 */
	public <T extends BaseModel> List<T> executeQuery(QueryBuilder query, Class<T> clazz)
	...
	private void closeConnection() throws QueryExecutorException {
		try {
			...
			throw new QueryExecutorException(ErrorCode.DATABASE_CONNECTION_ERROR, "Failed to close database connection",
					e);
		}
	}
	...
	private <T> void populateNestedFields(ResultSet resultSet, T instance, Set<String> columnNames) throws Exception {
		for (Field field : instance.getClass().getDeclaredFields()) {
	...
	private Object determineGroupingKey(ResultSet resultSet) {
		try {
			...
			System.err.println("Error determining grouping key" + e.getMessage());
			return null;
		}
	}
